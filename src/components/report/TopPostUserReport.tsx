...
type TopPostUserReportProps = {
  user: RouterOutputs["user"]["getById"];
  type: "VOTES" | "COMMENTS";
};

const TopPostUserReport = ({ user, type }: TopPostUserReportProps) => {
  const [limit, setLimit] = useState<number>(5);

  const { data: topPostList } = trpc.post.getByUserIdCursor.useQuery({
    limit: limit,
    userId: user.id,
    orderBy: type === "VOTES" ? "votesDesc" : "commentsDesc",
  });
...
export default TopPostUserReport;
